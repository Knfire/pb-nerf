...
def script():
    # convert images, cameras and SMPL to pb-nerf format
    if not check_exists(['annots.npy', 'smpl_params', 'smpl_vertices']):
        cmd = f'''python tools/easymocap2pb-nerf.py --data_root {data_root} --model_path {smpl_model_path} --regressor_path {smpl_regressor_path} --ranges {st} {ed} {interval}'''
        run_cmd(cmd)
    else:
